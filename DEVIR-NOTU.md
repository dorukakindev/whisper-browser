...
Güncel çalışma: [2026-09-25 16:30 — R124 Adım 3 (Faz B): tek-satır üst çerçeve (sekmeler başlıkta), birleşik Whisper düğmesi, 32px durum hapı, gruplu ⋯ menüsü; içerik y=88px ≤90 ölçümü](docs/devir/2026-09-25-1630.md).

Önceki çalışma: [2026-09-25 15:37 — R124 Adım 2 (Faz A): hayalet araç çubuğu, site çipi (kilit+host popover'a reklam koruması), aktif sekme yüzeyle birleşimi, hata ekranı katlanır detaylar](docs/devir/2026-09-25-1537.md).

Önceki çalışma: [2026-09-25 14:55 — R124 Adım 1: main→renderer dinamik mesajlar messageKey+params ile İngilizce; teşhis kataloğu iki dilli; "Altyazı · N" çipi locale; 401/403 metni genelleştirildi](docs/devir/2026-09-25-1455.md).
...
Önceki çalışma: [2026-09-23 18:19 — R117 UI polish paketi: ertelenen R115 maddeleri — "Altyazı işlemleri ▾" menüsü, up-next 5 sn geri sayım kartı (iptal/şimdi-oynat + tüm kapatma yolları), adres çubuğu domain-bold overlay (http/https şema dim + host bold), hız select → popup menü (select gizli durum deposu, menuitemradio ayna); öncelikli collapse ÖLÇÜMLE reddedildi (940px minWidth + gizli .browser-action-source → overflow fiziksel imkansız, ölü kod geri çıkarıldı); hex-token konsolidasyonu 440/334 ölçümü + 3 aşamalı planla ertelendi; tam npm test yeşil; BROWSER_BUG_REPORT_117](docs/devir/2026-09-23-1819.md).

Önceki çalışma: [2026-09-23 16:34 — R116: tarayıcı ⋯ menüsüne "Oynatıcıda izle" (YouTube → uygulama oynatıcısı, konum sürdürme + tarayıcıda duraklatma, gerçek-Electron kanıtlı); "Cümleleri birleştir" artık yalnız-çeviri yolunda da uygulanıyor (eski vaat boştu); çeviri kurtarma "loop" algısının kökü bulundu — paketler parça işçisinde seri bekliyordu → paylaşımlı rescue_pool ile paralel (233 sn tek-parça kurtarma sınıfı daralttı); BROWSER_BUG_REPORT_116](docs/devir/2026-09-23-1634.md).

Önceki çalışma: [2026-09-23 18:30 — R115 UI/UX denetim düzeltmeleri: doğrulanan tüm kusurlar düzeltildi (açık temada görünmez kontrol ikonları + 4 çakışan kural → tek koyu-cam blok, pasif kare artefaktı, iki-satır taşma → 38px tek satır + ⋯ menüsü, Esc zinciri + Backspace çıkışı, 85+ eksik ui-locale anahtarı, altyazısız panel boş-durum kartı, başlık 3-grup ayraç + ⋯, video zemini siyah, sayfa-çevirisi menüsü "Gelişmiş ayarlar", ok/Esc klavye gezinmesi + 120ms fade, adres select-all); "karanlık tema yok" ve "‹⟳› etiketsiz" iddiaları yanlış çıktı; öncelikli collapse/token konsolidasyonu/up-next kartı ertelendi; BROWSER_BUG_REPORT_115](docs/devir/2026-09-23-1830.md).

Önceki çalışma: [2026-09-23 14:05 — R114 gerçek uçtan-uca doğrulama: tiny-model E2E transkripsiyon ilk kez gerçek koştu (indirme→ffmpeg→motor→SRT→oynatıcı), canlı HLS rolling-playlist gauntlet'i sıfır stall geçti (discontinuity/gecikme/404-retry/ENDLIST + canlı altyazı yakalama), gerçek Windows'ta Electron+transkripsiyon+taskkill kapanış kanıtlı (GPU'suz VM); ürün bug'ı yok, ortam bulguları raporda; BROWSER_BUG_REPORT_114](docs/devir/2026-09-23-1405.md).

Önceki çalışma: [2026-09-23 03:58 — R113 hardening: 2 test-altyapı bug'ı düzeltildi (subtitle-output taskkill.exe askısı; tools-design narrow-panel flake); T2 hız/görünürlük genişlendi (hidden-donma tasarımı ölçüldü), T5 çapraz-sekme sızma smoke'u eklendi (0 sızma, sekme mutasyonlarının invoke-yanıtı senkronu dokümante), 400+1300 döngü soak GEÇTİ; BROWSER_BUG_REPORT_113](docs/devir/2026-09-23-0358.md).
...
Önceki çalışma: [2026-09-22 01:57 — T1 altyazı↔görünür katman zaman doğruluğu: üç overlay zamanlama hatası (bayat sınır timer'ı, rVFC açlığı, sınır epsiloni) kırmızı→yeşil kanıtla düzeltildi; BROWSER_BUG_REPORT_98](docs/devir/2026-09-22-0157.md).
...
Önceki çalışma: [2026-09-19 00:40 — Oynatıcı ayar panelleri görsel düzenleme: flex-gap düzeni, stilsiz URL input'u sarıldı, OAuth grubu, inline margin temizliği](docs/devir/2026-09-19-0040.md).
...
Önceki çalışma: [2026-09-18 10:10 — 20 ajanlık kapsam taraması bulguları (rapor 64)](docs/devir/2026-09-18-1010.md).
...
Önceki çalışma: [2026-09-17 21:17 — rapor 53 B/D doğrulama turu ve Electron kabulü (rapor 56)](docs/devir/2026-09-17-2117.md).
...
Önceki çalışma: [2026-09-16 10:38 — 10-ajan derin denetim turu (rapor 42)](docs/devir/2026-09-16-1038.md).
...
Önceki çalışma: [2026-09-16 08:31 — Great Courses gerçek-site CEA kabulü ve doğrudan tam yakalama](docs/devir/2026-09-16-0831.md).
...
Önceki çalışma: [2026-09-15 12:04 — canlı DASH ve uçtan uca altyazı bütünlüğü](docs/devir/2026-09-15-1204.md).
...
Önceki çalışma: [2026-09-15 07:49 — altyazı bütünlüğü, video kimliği ve artımlı çeviri](docs/devir/2026-09-15-0749.md).
...
Önceki çalışma: [2026-09-14-1544 — Çeviri düzenleme, arşiv ve bellek tutarlılığı](docs/devir/2026-09-14-1544.md)
