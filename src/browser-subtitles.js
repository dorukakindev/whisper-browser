...
function parseYoutubeCaptionMetadata(body, rawUrl = '') {
  let data;
  try { data = JSON.parse(String(body || '')); } catch (_) {
    data = {};
  }
  let params = null;
  try { params = new URL(String(rawUrl || '')).searchParams; } catch (_) {}
  const automatic = params?.get('kind') === 'asr'
    || data.kind === 'asr' || data.trackKind === 'asr'
    || data.captionTrack?.kind === 'asr' || data.isAutoGenerated === true;
  const translatedByService = !!params?.get('tlang') || data.translatedByService === true;
  const list = data.translationLanguages || data.translations
    || data.captionTrack?.translationLanguages || [];
  const translationLanguages = (Array.isArray(list) ? list : []).map((item) => ({
    languageCode: String(item?.languageCode || item?.code || item?.id || '').slice(0, 24),
    languageName: String(item?.languageName?.simpleText || item?.languageName
      || item?.name || '').slice(0, 80),
  })).filter((item) => item.languageCode).slice(0, 200);
  return { automatic, translatedByService, translationLanguages };
}
...
function parseMp4TimedSamples(buffer, matcher = {}) {
  const data = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || '');
      ...
      for (const run of runs) {
        ...
        for (const sample of run.samples) {
          ...
          samples.push({
            ...
            buffer: data.subarray(mediaCursor, sampleEnd),
            trackId: fragmentDefaults.trackId,
          });
          mediaCursor = sampleEnd;
          decodeTime += sample.duration;
        }
...
function parseMp4WebVtt(buffer, matcher = {}) {
  return normalizeCues(parseMp4TimedSamples(buffer, matcher).map((sample) => ({
    start: sample.start,
    end: sample.end,
    text: mp4PaylText(sample.buffer),
  })).filter((cue) => cue.text));
}

function parseMp4Stpp(buffer, matcher = {}) {
  const cues = [];
  for (const sample of parseMp4TimedSamples(buffer, matcher)) {
    // stpp örneği tam bir TTML belge/fragment taşır. Önündeki imza veya null
    // dolguyu XML başlangıcına kadar bırak; genel MP4 baytlarını metin sanma.
    const raw = sample.buffer.toString('utf8').replace(/\0+$/g, '');
    const xmlStart = raw.search(/<\?xml\b|<(?:[\w.-]+:)?tt\b/i);
    if (xmlStart < 0) continue;
    const parsed = parseXml(raw.slice(xmlStart));
    if (!parsed.length) continue;
    const duration = Math.max(0, sample.end - sample.start);
    const localTimeline = cuesUseLocalSegmentTimeline(parsed, duration, sample.start);
    for (const cue of parsed) {
      const start = localTimeline ? cue.start + sample.start : cue.start;
      const end = localTimeline ? cue.end + sample.start : cue.end;
      // Bozuk veya yanlış track'e ait XML'in saatler sonrasına taşmasını engelle.
      if (start < sample.start - 0.5 || start > sample.end + Math.max(1, duration)) continue;
      cues.push({ ...cue, start, end: Math.min(end, sample.end + Math.max(1, duration)) });
    }
  }
