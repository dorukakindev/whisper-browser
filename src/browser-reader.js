...
function buildBrowserReaderScript(action = 'toggle', rawPreferences = {}) {
  ...
  return `(() => {
    ...
    const copiedTitle = article.querySelector?.('h1');
    if (copiedTitle && normalizeHeading(copiedTitle.textContent) === normalizeHeading(title)) copiedTitle.remove();
    const previousOverflow = { root: document.documentElement.style.overflow, body: document.body?.style.overflow || '' };
    ...
    style.textContent = ':host{all:initial}.shell,.shell *,.shell *::before,.shell *::after{box-sizing:border-box}.shell{width:100%;min-height:100%;background:#0b0f13;color:#e8e4dc;font-family:Georgia,Cambria,serif;overflow-x:hidden}.bar{position:sticky;top:0;z-index:4;display:flex;flex-wrap:wrap;align-items:center;gap:8px 10px;padding:10px clamp(12px,2.4vw,18px);background:rgba(11,15,19,.96);border-bottom:1px solid #2a3038;font:13px Segoe UI,sans-serif}.bar strong{flex:1 1 180px;min-width:0;color:#d5a35c;letter-spacing:.04em;overflow-wrap:anywhere}.bar button{flex:0 0 auto;min-height:32px;border:1px solid #3a414b;background:#171c22;color:#e8e4dc;border-radius:6px;padding:6px 10px;cursor:pointer}.bar button:hover{border-color:#d5a35c}.bar button:focus-visible{outline:2px solid #d5a35c;outline-offset:2px}.layout{display:grid;grid-template-columns:minmax(0,1fr) 220px;gap:clamp(24px,4vw,36px);width:100%;max-width:calc(var(--reader-width) + 280px);margin:0 auto;padding:clamp(30px,6vw,54px) clamp(16px,3vw,28px) 90px}.article{min-width:0;max-width:var(--reader-width);font-size:var(--reader-font);line-height:var(--reader-line);overflow-wrap:anywhere}h1{font-size:clamp(1.75em,6vw,2.25em);line-height:1.12;margin:0 0 12px;color:#fff}h2,h3,h4{scroll-margin-top:108px;color:#f2eee7;line-height:1.25;margin:1.7em 0 .5em}p,li{margin:.8em 0}blockquote{border-left:3px solid #d5a35c;margin:1.3em 0;padding:.2em 1.1em;color:#c9c5bd}pre{max-width:100%;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;padding:14px;background:#12171d;border:1px solid #2a3038;border-radius:8px}code{font-family:Consolas,monospace}img{display:block;max-width:100%;height:auto;margin:1.4em auto;border-radius:7px}a{color:#72c7d5}.meta{font:13px Segoe UI,sans-serif;color:#9ca5ae;margin-bottom:32px}.toc{position:sticky;top:108px;align-self:start;max-height:calc(100vh - 126px);overflow:auto;border-left:1px solid #2a3038;padding-left:18px;font:13px/1.45 Segoe UI,sans-serif}.toc strong{display:block;color:#d5a35c;margin-bottom:10px}.toc a{display:block;color:#aeb5bd;text-decoration:none;padding:4px 0}.toc a:hover{color:#fff}.toc a:focus-visible{outline:2px solid #d5a35c;outline-offset:2px;color:#fff;border-radius:3px}.toc a[aria-current=true]{color:#fff;font-weight:600}.toc a[aria-current=true]::before{content:"\\2022 ";color:#d5a35c}@media(max-width:880px){.layout{display:block}.toc{display:none}}@media(max-width:520px){.bar strong{flex-basis:100%}.bar{align-items:stretch}.bar button{flex:1 1 auto}.layout{padding-inline:14px}}';
    ...
    const controller = {
      ...
      close() { const result = { ok: true, active: false, preferences: { ...state } }; host.remove(); document.documentElement.style.overflow = previousOverflow.root; if (document.body) document.body.style.overflow = previousOverflow.body; delete globalThis[KEY]; return result; },
    };
