...
function browserMediaCandidateRank(item) {
  ...
  const audible = item.muted !== true && (!Number.isFinite(volume) || volume > 0);
  ...
  const tier = playing && area > 16 && audible ? 7
    : playing && audio && audible ? 6
      : area > 16 && audible ? 5
        : playing && area > 16 ? 4 : playing && audio ? 3 : area > 16 ? 2 : playing ? 1 : 0;
