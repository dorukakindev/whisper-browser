...
- **Forcing a feed failure** (to exercise SmartTube empty/error fallbacks): temporarily edit `backend/invidious.py` `DEFAULT_INSTANCES` to `["https://127.0.0.1:9"]` (dead URL). The backend spawns per IPC call, so the change takes effect on the next feed fetch WITHOUT an app restart — the UI's "Tekrar dene"/retry button is enough to re-fire. yt-dlp fallback also fails via bot-check → feed returns ok+empty → the empty-videos fallback path fires (not the error-throw path — backend degrades to empty). REVERT the edit when done; the feed recovers on the next retry, which also proves the retry-success path.
