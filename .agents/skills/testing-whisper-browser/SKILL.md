...
- Preview renders a "Çeviri"-labeled TR line under each EN segment via `syncPreviewTranslation`. Known quirk: refine-pass revisions may NOT reach the preview (refresh payload has no index; `applyPreviewTranslations` skips duplicated time keys) — the output `.tr.srt` file is the ground truth.
