...
test('başlık ve koleksiyon içinde arar', () => {
  api.upsertWatchItem({ key: 'file:b', title: 'Mitoloji Dersi', collections: ['Felsefe'] });
  assert(api.searchWatchLibrary('mitoloji').some((x) => x.key === 'file:b'), 'başlık bulunmadı');
  assert(api.searchWatchLibrary('felsefe').some((x) => x.key === 'file:b'), 'koleksiyon bulunmadı');
});

test('altyazı metnini bulur ve zamanını döndürür', () => {
  ...
  const item = api.searchWatchLibrary('nadir')[0];
...
test('ASS altyazısında metni ve Dialogue zamanını bulur', () => {
  ...
  const item = api.searchWatchLibrary('özel ifade').find((x) => x.key === 'file:d');
...
test('saat alani olmayan VTT eslesmesinin zamanini döndürür', () => {
...
test('cp1254 altyazi oynaticiyla ayni sekilde aranir', () => {
  ...
  const item = api.searchWatchLibrary('ışık güneşi').find((x) => x.key === 'file:cp1254');
