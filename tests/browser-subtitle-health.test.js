...
let healthRenders=0;const tabError={browserTranslationLastError:'old'};
const clearCtx={player:{browserTranslationLastError:'API anahtarı girilmemiş.'},browserTabState:()=>tabError,renderBrowserSubtitleHealth:()=>{healthRenders++;}};
vm.createContext(clearCtx);vm.runInContext(source.slice(source.indexOf('function clearBrowserTranslationConfigError('),source.indexOf('async function saveTranslationProviderSettings(')),clearCtx);
