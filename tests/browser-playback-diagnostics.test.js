...
test('yalnız oynatma ile ilgili ağ kaynakları tanı matrisine girer', () => {
  ...
  assert.equal(inferPlaybackResource({ url: 'https://ads.test/login/frame', resourceType: 'subFrame' }), 'other');
...
test('üretim bağlantısı çok sekmeli resmi ağ/EME kanıt hattını kullanır', () => {
  const root = path.join(__dirname, '..');
  const main = fs.readFileSync(path.join(root, 'src', 'main.js'), 'utf8');
  const media = fs.readFileSync(path.join(root, 'src', 'browser-media-controller.js'), 'utf8');
  const renderer = fs.readFileSync(path.join(root, 'src', 'renderer', 'renderer.js'), 'utf8');
  const html = fs.readFileSync(path.join(root, 'src', 'renderer', 'index.html'), 'utf8');
  assert.match(main, /playbackDiagnostics: createPlaybackDiagnosticTracker\(\)/);
  assert.match(main, /installPlaybackWebRequestDiagnostics/);
  assert.match(main, /acceptDetails\(details\)[\s\S]{0,420}observedId === activeId/);
  ...
  assert.match(main, /widevineComponentStatus = \{ available: true, ready: false, failed: true,[^\n]+\};[\s\S]{0,180}reportBrowserDrmSupport/);
  assert.match(main, /media\.canPlayType\(videoType\)/);
  assert.match(main, /tab\.playbackDiagnostics\.observeMediaSample\(safeMedia\)/);
  assert.match(main, /tab\.playbackDiagnostics\.reset\(\{ clearCapabilities: true \}\)/);
  assert.match(main, /playbackDiagnostics: browserPlaybackSnapshot\(tab\)/);
