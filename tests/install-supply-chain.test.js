...
class FakeDriver {
  ...
  nodeVersion() { return this.options.nodeVersion || 'v22.12.0'; }
  systemPythonVersion() {
...
test('eski node reddedilir', (root) => expectCode(root, new FakeDriver(root, { nodeVersion: 'v22.11.0' }), 'NODE_VERSION'));
...
assert.equal(supportedNode('v22.12.0'), true);
assert.equal(supportedNode('v22.11.9'), false);
assert.equal(new RealDriver(SOURCE_ROOT, { platform: 'win32' }).npmInvocation().command, process.execPath);
assert.match(new RealDriver(SOURCE_ROOT, { platform: 'win32' }).npmInvocation().prefix[0], /npm-cli\.js$/);
