...
test('main import/export ve Python env çağrı yolları güvenli yardımcılara bağlıdır', () => {
  ...
  const spawnCount = [...main.matchAll(/\bspawn(?:Sync)?\s*\(/g)].length;
  const sanitizedSpawnCount = [...main.matchAll(/env:\s*withoutSecretEnv\(process\.env\)/g)].length;
  const transcribeSpawnCount = [...main.matchAll(/activeJob\s*=\s*spawn\(pythonPath, args, \{ env,/g)].length;
  assert.strictEqual(spawnCount, sanitizedSpawnCount + transcribeSpawnCount);
  assert.strictEqual(sanitizedSpawnCount, 11);
  assert.strictEqual(transcribeSpawnCount, 1);
});
...
test('secret sentinel yalnız child env içinde kalır; argv, export ve görünür sonuçlarda yoktur', () => {
  ...
  assert.strictEqual(imported.ignoredSecretCount, 3);
...
test('tarayıcı yedek URLsi basic auth, hassas sorgu ve fragment secretlarını taşımaz', () => {
  const main = fs.readFileSync(path.join(__dirname, '..', 'src', 'main.js'), 'utf8');
  const start = main.indexOf('const BROWSER_SENSITIVE_PARAMS');
  const end = main.indexOf('function readBrowserPlaces', start);
  assert(start >= 0 && end > start, 'tarayıcı URL temizleyicisi bulunamadı');
  const safeBrowserPlaceUrl = new Function(
    `${main.slice(start, end)}; return safeBrowserPlaceUrl;`,
  )();
