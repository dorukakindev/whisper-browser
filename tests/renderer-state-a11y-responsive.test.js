...
ok('tablist klavyesi ve kütüphane re-render odak/kaydırma geri yüklemesi bağlı', () => {
  ...
  // Master bu sekmeyi browserPlaceTabHistory adiyla tasiyor; sinanan sey AD degil,
  // tam sekme semantigi (role + aria-selected + aria-controls + roving tabindex).
  assert.match(html, /id="browserPlaceTabHistory"[^>]*role="tab"[^>]*aria-selected="false"[^>]*aria-controls="browserPlacesList"[^>]*tabindex="-1"/);
  // Master'da karsiligi sourceTabYoutube -> sourcePanelYoutube.
  assert.match(html, /id="sourceTabYoutube"[^>]*role="tab"[^>]*aria-selected="false"[^>]*aria-controls="sourcePanelYoutube"[^>]*tabindex="-1"/);
  // Degisken adi degil DAVRANIS sinaniyor: sekme etkinlestirilirken aria-selected
  // her seferinde guncellenmeli.
  assert.match(js, /\w+\.setAttribute\('aria-selected', \w+ \? 'true' : 'false'\)/);
  assert.match(js, /\w+\.setAttribute\('aria-expanded', \w+ \? '(?:true|false)' : '(?:false|true)'\)/);
});
