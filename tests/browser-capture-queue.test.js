...
test('pencere kapanışı yalnız bekleyen parçayı, sekme kapanışı ölçülemeyen kuyruğu da uyarır', () => {
  ...
  assert.match(main, /browser:tab:close[\s\S]{0,3400}browserTabCapturePending\(tab, true\)[\s\S]{0,360}browserCaptureCloseNeedsWarning\(captureStatus, true\)[\s\S]{0,180}confirmBrowserCaptureDiscard\(captureStatus\.pending, 'sekme', captureStatus\.unverified\)/,
