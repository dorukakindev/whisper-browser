...
assert.equal(parseSubtitlePayload(bracedAss.text, '', 'test.ass').cues[0].text,
  'Sahne {notu}\nalt satır');
const pathAss = buildBrowserSubtitleDocument([{ start: 0, end: 1, text: 'C:\\Users\\Name\\Note ve \\h' }], 'ass');
assert.equal(parseSubtitlePayload(pathAss.text, '', 'test.ass').cues[0].text,
  'C:\\Users\\Name\\Note ve \\h');
