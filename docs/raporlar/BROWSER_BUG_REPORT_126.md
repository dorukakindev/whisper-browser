...
| K1 | `MAX_MEDIA_TIME_SECONDS = 60*60*1000` — sn alanında ms formülü (~1000 saat) | Her iki dosyada `7 * 24 * 60 * 60` (7 gün — master'daki `report89` sözleşmesi 72 saatlik yayının korunmasını gerektiriyor; ilk denenen 24 saat o sözleşmeyi bozduğu için 7 güne çekildi) | `tests/browser-report126-regressions.test.js` testK1 (700000 sn → 604800 clamp + 72 saat korunur) |
