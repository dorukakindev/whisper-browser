# BROWSER BUG REPORT 122 — YouTube cihaz-kodu onay kaybı + çoklu hesap (SmartTube usulü)
...
- **BUG-122-02:** Google `refresh_token` döndürmezse (hesap daha önce bu istemciye izin vermişse ilk grant'ten sonra tekrar döndürmez) poll başarı emit'ini hata sayıyordu → geçici (yalnız access-token) oturum hiç kurulamıyordu.
- **BUG-122-03:** `poll` komutu genel `mediaJobs.youtube` slotunu kullanıyordu; 30 dakikaya kadar sürebilen bir poll, slotu kilitleyip browse/refresh çağrılarını aç bırakıyordu.
