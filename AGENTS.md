...
- Denetim/bug bulguları **her zaman bir `.md` dosyasına** kaydedilir; yalnız sohbette bırakma. Raporlar kökte DEĞİL `docs/raporlar/` altında tutulur (kök yalnız README, CHANGELOG, katkı/güvenlik ve tasarım sözleşmesi belgeleri içindir). Seri `BROWSER_BUG_REPORT_<sıra>.md` adlandırmasını izle (son: 121).
- **Bu projenin asıl kullanım ortamı Windows'tur (`D:\Whisper Local`).** Ubuntu'da geliştirmeye ve Electron testi yapmaya devam et, fakat her PR'da Windows etkisini ayrıca değerlendir: `install.bat` ve `start.bat` akışını, Windows yollarını, PowerShell/cmd davranışını, dosya seçiciyi, UTF-8/Türkçe karakterleri ve varsa GPU/CUDA bağımlılıklarını kontrol et. Uygulamanın normal Windows açılışı `start.bat` üzerindendir; yalnız `npm start` testi bunu doğrulamaz. Değişen alanlara yönelik testleri çalıştır; her görevde gereksiz yere tam test paketini koşma. Windows CI sonucunu bekle. Ubuntu Electron testi, Windows'ta gerçek arayüz, DPI, GPU, DRM veya kullanıcı hesabı doğrulaması yerine geçmez; doğrulayamadıklarını PR'da açıkça yaz. Güncel `master` üzerinden ayrı dal ve PR aç; kendin merge etme. Değişen dosyaları, test sonuçlarını, Windows'a özgü riskleri ve doğrulanmamış sınırları devir notuna yaz. Gerçek profil, API anahtarı ve kişisel verileri testlerde veya commit'lerde kullanma. PR'ı kullanıcı inceleyip Windows bilgisayara taşıyacaktır.
