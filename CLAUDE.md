...
**Test/doğrulama:** `npm test` tüm testleri çalıştırır (`tests/*.test.js` + `backend/test_transcribe.py`; venv yoksa Python kısmı atlanır). Node testleri Electron'a bağlı olmayan saf mantığı KAYNAKTAN kesip çalıştırır — sınır işaretleri değişirse test yüksek sesle kırılır. Ayrıca sözdizimi kontrolü:
```powershell
...
- **Altyazı düzenlemesi dosya BİÇİMİNİ korur:** `saveCueEdit` her biçimi `cuesToSrt` ile yazıyordu; `.ass` dosyasına düz SRT yazılıyor (stiller, konumlar, konuşmacı adları yok oluyor), `.vtt` de WEBVTT başlığını kaybediyordu. Artık: ass → `replaceAssDialogueText` ile YALNIZCA ilgili Dialogue satırının metin alanı değişir (ham metin `player.subRaw`'de tutulur), vtt → `cuesToVtt`, srt → `cuesToSrt`.
- **Dosyaya yazan her yol `backupOnce` + `writeSubtitleAtomic` kullanır** (bir kez .bak, sonra .tmp→rename). `subs:shift` ayrıca `decodeSubtitleBuffer` ile okur — düz utf-8 okumak eski cp1254 altyazıda ş/ğ/ı harflerini U+FFFD yapıp dosyaya geri yazıyordu.
- **Ön kontroller tek yerde (`optsProblem`):** tekil başlatma ve kuyruk AYNI fonksiyonu çağırır. Kuyruk eskiden hiç kontrol etmiyordu; anahtarsız çeviri/LLM/diarization veya geçersiz kırpma aralığıyla iş eklenebiliyordu.
- **Medya kimliği ve kuşak (`mediaKeyFor` / `player.generation`):** kaynak anahtarı ham URL DEĞİL kararlı kimliktir (`youtube:<videoId>` veya normalize yol) — aynı video youtu.be/ID, watch?v=ID&t=30 gibi adreslerde ayrı kayda düşmesin. Kaynak her değiştiğinde `generation` artar; süren her asenkron iş başladığı kuşağı saklayıp sonuç geldiğinde `staleGeneration()` ile kontrol eder. Yeni asenkron iş eklerken bu korumayı EKLE, yoksa eski videonun altyazısı yenisine bağlanır.
- **Probe metadatası kaynaktan SONRA uygulanır:** `setPlayerSource/setPlayerHls` 4. parametre olarak `meta` alır; `setMediaKey → resetMediaBoundState` her şeyi sildiği için bölümler ancak sıfırlamadan sonra uygulanabilir.
- **Kalite seçicisinin değeri HER ZAMAN piksel yüksekliğidir**; HLS seviye indeksi `option.dataset.level`'de durur. Değeri indekse çevirme — "İndir ve izle" onu yükseklik olarak okuyor (eskiden 1080p seçilince backend'e height=2 gidebiliyordu).
- **Altyazı düzenlemesi bellekten ÖNCE diske yazar:** `player.cues` ancak yazma başarılı olduysa güncellenir. VTT'de `replaceVttCueText`, ASS'te `replaceAssDialogueText` ile cerrahi düzenleme yapılır (STYLE/REGION/NOTE, cue kimlikleri, satır ayarları ve baştaki ASS konum/stil etiketleri korunur).
- **Zaman kodu yazarken önce tam ms'ye yuvarla, sonra parçala** — ayrı yuvarlama `00:00:01,1000` gibi geçersiz kod üretiyordu.
- **Altyazı sürükleme (`makeSubtitleDraggable`):** konum YÜZDE olarak saklanır (localStorage `subtitlePos`), böylece pencere boyutu ve tam ekran fark etmez. Overlay'de `pointer-events` artık tam ekranda da AÇIK — bu yüzden sürüklenmeyen tıklama elle videoya iletilir (yoksa tam ekranda altyazıya tıklayınca oynat/duraklat çalışmazdı). Kullanıcı konumu ayarlayınca sahneye `.sub-moved` eklenir ve "kontroller görününce yukarı kalk" kuralı devre dışı kalır — kullanıcının seçimi kazanır. 4 px eşiği tıklamayı sürüklemeden ayırır.
- **Konuşma başlangıcına yaslama (`snap_entries_to_speech`):** boru hattında `normalize_timings`'ten ÖNCE çalışır (min-süre/CPS kuralları yeni başlangıca göre uygulansın). Ölçüm: segment ve kelime damgaları AYNI çıkıyor (fark 0 ms) — yani sorun segment/kelime seçimi değil, damganın kendisinin sessizliğe taşması. Bu yüzden çözüm sesin kendisine yaslamak. Yaslama VAD'i transkripsiyondakinden DAHA HASSAS (`threshold=0.35, min_silence=150ms`) — yoksa tüm film birkaç dev bölge olur ve yaslama işe yaramaz. Saf mantık `snap_entries_to_regions`'da (test edilebilir olsun diye).
- **Çeviri bağlamı (`--translate-context`):** her parçanın önüne/arkasına N satır eklenir ve sistem promptundaki **## BAGLAM** bölümü modele bunları ÇEVİRMEMESİNİ söyler. Bu bölüm bir ara eksikti: bağlam gönderiliyor ama açıklanmıyordu. Bağlam kapalıysa (`0`) ne anahtarlar ne de o prompt bölümü gönderilir. `CONTEXT_LINES` prompt kurulumundan ÖNCE hesaplanmalı.
- **Sağ panel = okuma alanı (Trancy tarzı):** `.player-body` ızgarası `1fr 6px var(--side-w)`; genişlik `--side-w` ile mod (Sinema/Okuma/Çalışma) veya sürükleme tutamağı tarafından ayarlanır, localStorage'da saklanır. Eski ayar panelleri `.settings-drawer` içine taşındı (dişli düğmesi). Yeni ayar eklerken oraya koy, okuma akışını bölme.
- **Otomatik takip:** aktif satır RECT FARKIYLA ortalanır — `offsetTop` kullanma, `offsetParent` `.player-side` olduğu için kaydırma kutusuna göre değil. Kullanıcının kaydırması ZAMAN penceresiyle değil `player.expectedScroll` (beklenen konum) karşılaştırmasıyla ayırt edilir; zaman tabanlı sezgi, kullanıcının hemen ardından gelen otomatik kaydırma tarafından maskeleniyordu. Altyazı boşluklarında (aktif satır yokken) "Aktif satıra dön" düğmesi GİZLENMEZ.
- **Kart listesi ve uzun videolar:** `.cue-card` `content-visibility: auto` kullanır — binlerce blokta tarayıcı görünmeyen kartları çizmez. Kaynak ve çeviri `translationFor()` ile ZAMAN ÖRTÜŞMESİNE göre eşleştirilir (çeviri ayrı bir dosyadan gelmiş olabilir).
