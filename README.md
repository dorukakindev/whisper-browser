...
- **Cümleleri birleştir** (opsiyonel, varsayılan kapalı) — cümle bir sonraki altyazıda birkaç kelimeyle bitiyorsa iki blok tek blokta toplanır ve "…" devam işaretleri temizlenir. Çeviride **ayrıca** uygulanır: "…" işaretini çeviri modeli koyar (kaynakta hiç yoktur) ve Türkçede yüklem sona geldiği için cümle İngilizceden farklı yerden bölünür. Ölçüm (231 bloklu röportaj): çeviride 231 → 223 blok, şikâyet edilen satırlar 13 KPS ile tek cümle oldu; bedeli 8 sn üstü blok sayısının 0 → 5 çıkması — bu yüzden opsiyonel
- **Flaş blok düzeltmesi** — 0,8 sn altında kalan (okunamayan) altyazılar komşusuyla birleştirilir. Bunlar `normalize_timings` ile uzatılamıyor: tavan "sonraki başlangıç − boşluk" ve bloklar bitişik olduğunda uzatacak yer kalmıyor. Ölçüm: 0,8 sn altı blok kaynakta 11 → 2, çeviride 11 → 0
...
- **Çeviri önbelleği** — çevrilen her blok saklanır; aynı videoyu tekrar çalıştırınca veya birkaç satırı düzeltince **yalnızca değişenler** API'ye gider. Ölçüm: 50 bloklu dosyada ikinci çalıştırma **0 istek**, 3 satır değişince **1 istek**. Hedef dil veya model değişirse önbellek paylaşılmaz
- **Çeviri oluştur (Whisper'sız)** — oynatıcıda yüklü altyazıyı olduğu gibi çevirir: ses indirme yok, model çalışmaz, **zaman kodlarına dokunulmaz**. YouTube'dan indirdiğiniz veya elinizde olan bir SRT'yi tek tuşla Türkçeye çevirip ikinci altyazı olarak izleyin
- **Yayın ses parçası (dublaj)** — indirmeden izlerken birden çok ses parçası varsa oynatma sırasında değiştirilir; altyazı ve kalite seçimini etkilemez
...
- **İzlerken cümle birleştirme** — oynatıcı ayarlarındaki anahtar yüklü altyazıya **anında** uygulanır ve dosyayı değiştirmez; kapatınca ham hâline döner. Yeni altyazı üretirken de geçerli olsun isterseniz Gelişmiş ayarlardaki aynı seçeneği açın
- **Gizlenebilir araç çubuğu** — oynatıcının altındaki cümle araçları tek tıkla katlanır (durum hatırlanır); "Aktif satıra dön" artık altyazının üstünde yüzmüyor, araç şeridinde duruyor
- **Kendi başlık çubuğu** — işletim sisteminin "Whisper Altyazı" başlığı gizli; pencere düğmeleri overlay olarak kalır, başlık alanından sürüklenir
- **AI sohbet paneli** (oynatıcıda, "AI'ye sor" sekmesi) — izlerken video hakkında serbestçe soru sorabilir, konuşmayı sürdürebilirsiniz. Her soruya o anki altyazı satırı, mevcut çevirisi, üç önceki/sonraki satır, zaman ve video başlığı eşlik eder; önceki turlar hatırlanır (son 8 mesaj). Hazır sorular tek tıkla: *bu cümle ne demek*, *deyim var mı*, *çeviri doğru mu*, *kısaca özetle*
- **Tek tıkla altyazı + çeviri** — oynatıcının alt çubuğundaki düğme; Whisper'ı çalıştırıp altyazıyı çıkarır, aynı işte çevirir ve biter bitmez kaynağı birincil, çeviriyi ikincil altyazı olarak yükler. Kalıcı çeviri ayarınıza dokunmaz
